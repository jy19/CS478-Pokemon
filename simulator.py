        ...
        if type == "faint":
            ...
            if player == 1:
                ...
                if move not in known_moves:
                    ...
                    guess_moves = [x[0] for x in poke.predict_moves(known_moves) if x[0] not in known_moves][
                                  :4 - len(known_moves)]
...
class Action:
    ...
    def __eq__(self, other):
        ...
        if self.type == "move":
            return (
                self.move_index == other.move_index and self.backup_switch == other.backup_switch and self.mega == other.mega and self.volt_turn == other.volt_turn)
        if self.type == "switch":
    ...
    def __repr__(self):
        if self.type == "move":
            if self.volt_turn is None:
                ...
                return "%s(%u, %s, %s, %u)" % (
                    self.type, self.move_index, self.backup_switch, self.mega, self.volt_turn)
